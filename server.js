  ...
  gameState.projects.forEach((project, projectIndex) => {
    ...
    gameState.players.forEach((player, playerIndex) => {
      if (player.bids && player.bids[projectIndex] && player.bids[projectIndex].length > 0) {
        ...
        // Calculate how much the bid exceeds requirements for tiebreaking
        let relevantValue = 0;
        if (project.cost) {
          Object.keys(project.cost).forEach(type => {
            relevantValue += totalEnergy[type] || 0;
          });
        }
        ...
        bidsByPlayer.push({
          ...
          relevantValue: relevantValue  // Total of required resource types
        });
      }
    });
    ...
    if (validBids.length > 0) {
      // Sort by relevant resources first, then total value as tiebreaker
      validBids.sort((a, b) => {
        // First compare by relevant resources (those required by the project)
        if (b.relevantValue !== a.relevantValue) {
          return b.relevantValue - a.relevantValue;
        }
        // If tied on relevant resources, use total value as tiebreaker
